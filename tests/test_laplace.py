...
from research_workspace.laplace_cli import LaplaceError, _ingest, _project_from_dir, detect_project, init_laplace, main
...
def test_laplace_init_registry_and_parent_detection(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_laplace_registry_conflict_is_explicit(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _home(monkeypatch, tmp_path / "home")
...
def test_laplace_ingest_dry_run_rejects_escape(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _home(monkeypatch, tmp_path / "home")
...
def test_laplace_init_refuses_application_repository(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    _home(monkeypatch, tmp_path / "home")
...
def test_laplace_validate_cli(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    _home(monkeypatch, tmp_path / "home")
    root = Path(init_laplace("Demo", cwd=tmp_path)["project"])
