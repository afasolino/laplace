"""Durable, owner-bound result artifacts with bounded byte-exact paging."""
...
class ResultStore:
    ...
    def __init__(self, root: Path) -> None:
        ...
        if self.root.is_symlink() or not self.root.is_dir():
            raise ResultStoreError("zetsu_result_root_invalid")
        if os.name != "nt":
    ...
    def _directory(self, result_id: str) -> Path:
        if _RESULT_RE.fullmatch(result_id) is None:
            raise ResultStoreError("zetsu_result_id_invalid")
        target = self.root / result_id
        if target.exists() and (target.is_symlink() or not target.is_dir()):
            raise ResultStoreError("zetsu_result_directory_invalid")
        return target

    @staticmethod
    def _artifact_name(name: str) -> str:
        if _ARTIFACT_RE.fullmatch(name) is None:
            raise ResultStoreError("zetsu_result_artifact_invalid")
        return name
            ...
            with os.fdopen(descriptor, "wb") as output:
                ...
                while True:
                    ...
                    if not isinstance(block, bytes):
                        raise ResultStoreError("zetsu_result_artifact_not_binary")
                    output.write(block)
                    digest.update(block)
                    size += len(block)
                output.flush()
    ...
    def stage_stream(self, session_id: str, name: str, source: BinaryIO) -> Path:
        ...
        if staging.is_symlink() or staging.resolve() != staging:
            raise ResultStoreError("zetsu_result_staging_invalid")
        ...
        for raw_name, source in sorted(artifacts.items()):
            ...
            if isinstance(source, Path):
                try:
                    ...
                    raise ResultStoreError("zetsu_result_source_unavailable") from exc
                if stat.S_ISLNK(details.st_mode) or not stat.S_ISREG(details.st_mode):
                    raise ResultStoreError("zetsu_result_source_invalid")
    ...
    def _manifest(self, result_id: str) -> tuple[Path, JsonObject]:
        ...
        try:
            if manifest_path.is_symlink():
                raise ResultStoreError("zetsu_result_manifest_invalid")
            raw: object = json.loads(manifest_path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as exc:
            raise ResultStoreError("zetsu_result_not_found") from exc
    ...
    def page(
        ...
        *,
        ...
        if isinstance(offset, bool) or not isinstance(offset, int) or offset < 0:
            raise ResultStoreError("zetsu_result_offset_invalid")
        if (
            isinstance(max_bytes, bool)
            or not isinstance(max_bytes, int)
            or not 1 <= max_bytes <= _MAX_PAGE_BYTES
        ):
            raise ResultStoreError("zetsu_result_page_size_invalid")
        ...
        if (
            ...
            raise ResultStoreError("zetsu_result_not_found")
        name = self._artifact_name(artifact)
        artifacts = manifest.get("artifacts")
        record = artifacts.get(name) if isinstance(artifacts, dict) else None
        if not isinstance(record, dict):
            raise ResultStoreError("zetsu_result_artifact_not_found")
        target = directory / name
        try:
            details = target.lstat()
        except OSError as exc:
            raise ResultStoreError("zetsu_result_artifact_not_found") from exc
        if (
            ...
            raise ResultStoreError("zetsu_result_artifact_identity_changed")
